...
async def get_trees(
    ...
    if order_by:
        ...
        elif order_by == OrderClause.HEIGHT_ASC:
            query = query.order_by(asc(TreeModel.height_cm))
        elif order_by == OrderClause.HEIGHT_DESC:
            query = query.order_by(desc(TreeModel.height_cm))
...
async def get_tree_by_id(
    ...
    # Створюємо запит для отримання дерева за id
    query = select(TreeModel).where(TreeModel.id == tree_id)
    result = await db_session.execute(query)
    tree = result.scalar_one_or_none()

    # Якщо дерево не знайдено, кидаємо помилку
    if not tree:
        raise HTTPException(status_code=404, detail="Tree not found")
